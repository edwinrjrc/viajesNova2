...
public class ConsolidadorMBean extends BaseMBean {
	...
	private final static Logger logger = Logger.getLogger(ConsolidadorMBean.class);
	/**
	 * 
	 */
	...
	public void ejecutarMetodo(){
		try {
			...
			if (this.isNuevoConsolidador()){
				...
				this.getConsolidador().setIpCreacion(this.obtenerRequest().getRemoteAddr());
				this.setShowModal(this.negocioServicio.ingresarConsolidador(getConsolidador()));
				this.setTipoModal("1");
				this.setMensajeModal("Consolidador ingresado Satisfactoriamente");
			}
			else {
				this.getConsolidador().setUsuarioModificacion(usuario.getUsuario());
				this.getConsolidador().setIpModificacion(this.obtenerRequest().getRemoteAddr());
				this.setShowModal(this.negocioServicio.actualizarConsolidador(getConsolidador()));
				this.setTipoModal("1");
	...
	public void consultarConsolidador(Integer idConsolidador){
		try {
			...
			this.setConsolidador(this.negocioServicio.consultarConsolidador(consolida));
		} catch (SQLException e) {
			logger.error(e.getMessage(), e);
			this.setShowModal(true);
			this.setTipoModal("2");
	...
	public List<Consolidador> getListaConsolidador() {
		try {
			listaConsolidador = this.negocioServicio.listarConsolidador();
	...
	/**
	 * @param listaConsolidador the listaConsolidador to set
	 */
	...
	/**
	 * @param nuevoConsolidador the nuevoConsolidador to set
	 */
	...
	/**
	 * @param editarConsolidador the editarConsolidador to set
	 */
	...
	/**
	 * @param negocioServicio the negocioServicio to set
	 */
